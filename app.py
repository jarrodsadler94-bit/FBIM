...
PERSISTENT_KEYS = [
    ...
    "receipt_type", "dispatch_type", "response_type", "travel_to_site_mins", 
...
detection_type = st.radio("Detection Type", ["Automatic", "Manual"], key="detection_type")
detection_time = st.number_input("Fire detection time (seconds)", min_value=0, value=0, key="detection_time")
...
sys_type = st.selectbox("Select System Type / Floor Area", list(table_a_data.keys()), key="sys_type")
...
notification_type = st.radio("Notification Method", ["Automatic alarm signalling equipment (ASE)", "Manual call to Fire Brigade"], key="notification_type")

col1, col2 = st.columns(2)
with col1:
    verification_time = st.number_input("Time for verification (seconds)", min_value=0, value=0, key="verification_time")
with col2:
    notification_delay = st.number_input("Time Delay for fire brigade notification (seconds)", min_value=0, value=0, key="notification_delay")
...
if notification_type == "Manual call to Fire Brigade":
    ...
    receipt_type = st.selectbox("Select Information Receipt Method", list(table_c_data.keys()), key="receipt_type")
...
dispatch_type = st.selectbox("Select Dispatch Method", list(table_d_data.keys()), key="dispatch_type")
...
response_type = st.selectbox("Select Firefighter Turnout Activity", list(table_e_data.keys()), key="response_type")
...
with col3:
    travel_to_site_mins = st.number_input("Travel time to site (minutes)", min_value=0.0, value=0.0, step=0.1, key="travel_to_site_mins")
    travel_to_site_sec = int(travel_to_site_mins * 60)
with col4:
    travel_through_site_m = st.number_input("Travel distance through site (meters)", min_value=0, value=0, key="travel_through_site_m")
...
time_g = st.number_input("Time for any security procedure (seconds)", min_value=0, value=0, key="time_g")
...
warden_type = st.selectbox("Building Floor Area (for Warden Comm.)", list(table_h_data.keys()), key="warden_type")
time_h = table_h_data[warden_type]

# 9. Gain / Force Entry (Tables I & J)
st.subheader("9. Gain or Force Entry (Tables I & J)")
entry_method = st.radio("Entry Method", ["Open/Unobstructed (0s)", "Gain Entry with Keys (Table J)", "Force Entry (Table I)"], key="entry_method")
time_i_j = 0
...
if entry_method == "Force Entry (Table I)":
    ...
    entry_door = st.selectbox("Select door/gate type (Forced)", list(table_i_data.keys()), key="entry_door_forced")
    ...
    entry_door = st.selectbox("Select door/gate type (Keys)", list(table_j_data.keys()), key="entry_door_keys")
...
wayfinding_type = st.selectbox("Select Complexity/Building Size", list(table_k_data.keys()), key="wayfinding_type")
...
fdcie_type = st.selectbox("Building Floor Area (for FDCIE)", list(table_l_data.keys()), key="fdcie_type")
...
tool_type = st.selectbox("Select Tools Required", list(table_p_data.keys()), key="tool_type")
...
horiz_travel_type = st.selectbox("Select Horizontal Travel Conditions", list(table_q_data.keys()), key="horiz_travel_type")
horiz_distance = st.number_input("Horizontal Distance (meters)", min_value=0.0, value=0.0, key="horiz_distance")
...
stair_travel_type = st.selectbox("Select Stair Travel Conditions", list(table_t_data.keys()), key="stair_travel_type")
stair_flights = st.number_input("Number of Stair Flights", min_value=0, value=0, key="stair_flights")
steps_per_flight = st.number_input("Average Steps per Flight", min_value=0, value=15, key="steps_per_flight")
...
selected_safety = st.multiselect("Select required safety procedures", list(table_n_o_data.keys()), key="selected_safety")
...
water_search = st.selectbox("External Search for Street Main (Table W)", list(table_w_data.keys()), key="water_search")
...
time_x = 0
appliance_pos_dist = st.number_input("Position Appliance Distance (m) (X1)", min_value=0.0, value=0.0, key="appliance_pos_dist")
if appliance_pos_dist > 0:
    time_x += round(appliance_pos_dist / calc_speed(1.09, 0.48, stat_mode, 0.16), 1)

selected_static = st.multiselect("Select static water operations", list(static_water_ops.keys()), key="selected_static")
if "Remove suction hose & connect (X2, per 3m)" in selected_static:
    suction_lengths = st.number_input("Number of 3m suction hose lengths", min_value=1, value=1, key="suction_lengths")
...
time_y = 0
sec_search_area = st.number_input("Secondary Search Area (m2) (Y1)", min_value=0.0, value=0.0, key="sec_search_area")
if sec_search_area > 0:
    time_y += round(sec_search_area / calc_speed(0.16, 0.05, stat_mode, 0.07), 1)

rescue_dist = st.number_input("Remove/Rescue Person Distance (m) (Y2)", min_value=0.0, value=0.0, key="rescue_dist")
if rescue_dist > 0:
...
time_z = 0
aerial_pos_dist = st.number_input("Position Aerial Appliance Dist (m) (Z1)", min_value=0.0, value=0.0, key="aerial_pos_dist")
...
aerial_setup = st.selectbox("Appliance Setup Prep", list(aerial_setup_opts.keys()), key="aerial_setup")
time_z += calc_time(aerial_setup_opts[aerial_setup]["mean"], aerial_setup_opts[aerial_setup]["sd"], stat_mode)

if st.checkbox("Conduct Aerial Safety Procedures (Z3)", key="aerial_safety"):
...
aerial_elev = st.selectbox("Elevate & Manoeuvre", list(aerial_elevate_opts.keys()), key="aerial_elev")
elev_dist = st.number_input("Elevation Distance (m)", min_value=0.0, value=0.0, key="elev_dist")
...
if st.checkbox("Charge Monitor (Z5)", key="aerial_monitor"):
